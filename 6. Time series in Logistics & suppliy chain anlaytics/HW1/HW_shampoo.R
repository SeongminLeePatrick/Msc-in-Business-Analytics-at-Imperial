...
# l.start = L0 hat, b.start = T0 hat

plot(shampoo.HW)
lines(fitted(shampoo.HW2)[,2], col = "blue", lty = 2) ## second estimated model

# in-sample one-step forecast
sqrt(shampoo.HW$SSE/(length(shampoo)-2))
sqrt(shampoo.HW2$SSE/(length(shampoo)-2))
## result shows that we have to be carefule about selecting the starting value in HW
...
shampoo.ets2 
#Chekck, ETS(A, A, N), L0hat = 204.8691, T0hat = -2.6258
# the last letter 'N' we don't have seasonlaity,'A': additive seaosnality, 'm': mul,, seasonlaity
# first: error type / second: type of trend / third: type of seasonal factor
...
### model evaluation
# Fit model with first 2 years of data
...
# Do not adjust the residual test