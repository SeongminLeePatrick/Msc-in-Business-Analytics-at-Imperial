...
Q2 <- optimize(f, c(0, 800), p = 250, c = 100, s = 80, 
               d = rnorm(10000, mean = 350, sd = 150), maximum = T); Q2
# c(0, 800): search range

CSL <- mean(d <= Q2$maximum); CSL #CSL = P(demand <= Q) = E[1 D<=Q]
profit <- mean(p*pmin(Q2$maximum, d) + s*pmax((Q2$maximum - d), 0) - c*Q2$maximum); profit
overstock <- mean(pmax((Q2$maximum - d), 0)); overstock # expected overstock = E[(Q-d)+]
understock <- mean(pmax((d - Q2$maximum), 0)); understock # expected understock = E[(d-Q)+]
